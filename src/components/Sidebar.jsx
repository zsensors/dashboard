...
const Sidebar = () => {
  ...
  return (
       <div className='ml-3 h-screen md:overflow-hidden overflow-auto md:hover:overflow-auto pb-10 lg:border-r-8'>
            {activeMenu && (<>
                ...
                <div className='mt-10'>
                    {links.map((item) => (
                        <div key={item.title} >
                            <p className='text-gray-400 m-3 mt-4 uppercase border-b-4 '>
                            {item.title}
                            </p>
                            {item.links.map((link) => (
